...
const getAll = function (searchParams, done) {
    ...
    if (status && status !== "" && status !== "all") {
        if (status == "upcoming") {
            sql += ` AND auction.auction_startingdate > CURRENT_TIMESTAMP()`;
        }
        if (status == "active") {
            sql += ` AND auction.auction_startingdate <= CURRENT_TIMESTAMP() AND auction.auction_endingdate > CURRENT_TIMESTAMP()`;
        }
        if (status == "expired") {
            sql += ` AND auction.auction_endingdate <= CURRENT_TIMESTAMP() AND (bid.bid_amount < auction.auction_reserveprice OR bid.bid_amount IS NULL)`;
        }
        if (status == "won") {
            sql += ` AND auction.auction_endingdate <= CURRENT_TIMESTAMP() AND bid.bid_amount >= auction.auction_reserveprice`;
        }
    }
...
/**
 Get won auctions
 */
const get_won_auctions = function (user_id, done) {
    let sql = "SELECT auction.auction_id AS id, \
            category.category_title AS categoryTitle, \
            category.category_id AS categoryId,\
            auction.auction_title AS title, \
            auction.auction_reserveprice AS reservePrice, \
            auction.auction_startingdate AS startDateTime,\
            auction.auction_endingdate AS endDateTime, \
            bid.bid_amount AS currentBid, \
            auction.auction_startingprice AS startingPrice \
            FROM auction, bid, category \
            WHERE auction.auction_id = bid.bid_auctionid \
            AND auction.auction_categoryid = category.category_id \
            AND bid.bid_userid = " + user_id + "\
            AND auction.auction_endingdate < CURRENT_TIMESTAMP \
            AND bid.bid_amount = (SELECT MAX(bid.bid_amount) FROM bid WHERE bid.bid_auctionid = auction.auction_id) \
            AND bid.bid_amount >= auction.auction_reserveprice";
    db.get_pool().query(
        sql,
        function (err, results) {
            if (err) {
...
module.exports = {
    ...
    getWonAuctions: get_won_auctions
}
